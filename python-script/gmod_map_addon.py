...
def main():
    ...
    gamemode_file = gamemodes_dir / safe_name / f"{safe_name}.txt"
    gamemode_file.parent.mkdir(parents=True, exist_ok=True)
    gamemode_file.write_text(
        f'"{safe_name}"\n{{\n  "title" "{dir_name}"\n  "maps" "{maps_str}"\n  "menusystem" "0"\n  "source" ""\n}}\n'
    )
    ...
    if not any(data_static.iterdir()):
        data_static.rmdir()
