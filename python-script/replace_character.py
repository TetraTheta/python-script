...
class ReplaceCharacterNamespace(Namespace):
    paths: list[str]
    reverse: bool
...
def smart_replace(text: str) -> str:
    ...
    # Opening double quotes are detected by their left-side context.
    text = re.sub(r'(^|[\s(\[{])"', lambda m: m.group(1) + "\u201c", text, flags=re.MULTILINE)
    text = re.sub(r'"', "\u201d", text)

    # Apostrophes between word characters should not become opening quotes.
...
def smart_unreplace(text: str) -> str:
    reverse_map = [
        ("«", "<<"),
        ("»", ">>"),
        ("⇐", "<="),
...
def transform_file(file_path: Path, reverse: bool = False) -> None:
...
""",
    ...
    parser.add_argument(
        ...
        nargs="+",
        metavar="PATH",
        help="Files or directories to process (.txt and .md only)",
    )
    parser.add_argument(
        "-r",
        "--reverse",
    ...
    args = parser.parse_args(namespace=ReplaceCharacterNamespace())
